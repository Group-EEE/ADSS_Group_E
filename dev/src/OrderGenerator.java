...
/**
 * The class OrderGenerator ask the user for products that he want to invite.
...
public class OrderGenerator{
    ...
    static Map<Supplier, Order> bestSuppliersCombination; // The cheapest combination of orders from suppliers
    static Map<Supplier, Order> MustBuyOrders; // When we want to buy a certain quantity of certain product, and there is no supplier that can supply the desired quantity, We must buy from the supplier that can supply the maximum quantity. This map includes all the orders like mention

    static List<Product> ProductsInOrder ; // contain all the products in the order
    ...
    public static void createNewOrder() {
        ...
        ProductsInOrder = new ArrayList<Product>(); // contain all the products in the order
        ...
        MustBuyOrders = new HashMap<Supplier, Order>();
        bestSuppliersCombination = new HashMap<Supplier, Order>();
     ...
     */
    private static void buildProductsList(){
    ...
    private static List<SupplierProduct> findSuppliersThatCanSupplyTheProduct(List<SupplierProduct> suppliersProduct, int quantity,
        ...
        if(suppliersProduct.size() == 0)
            throw new RuntimeException("Can not supply the desired quantity for the product: " + product +
        ...
        if(suppliersThatCanSupplyTheProduct.size() == 0){ // If there is no supplier that can supply the desired quantity
            ...
            if (!MustBuyOrders.containsKey(maxSupplier)) // If we haven't opened an order to this supplier in the blackList yet
                MustBuyOrders.put(maxSupplier, new Order(maxSupplier)); // Open an order to this supplier in the blackList
            ...
            return findSuppliersThatCanSupplyTheProduct(newSuppliersProduct, quantity - maxProducts,
                    index, quantityForExc + maxProducts, product); // Call the function again. We want to find the list of suppliers that can supply the desired quantity **after** we order from the max supplier.
        }
    ...
    private static void UpdateTheBestOrder(int[] combination) {
        ...
        Map<Supplier, Order> orders = cloneMap(MustBuyOrders);
        for (int i = 0; i < combination.length; i++) { // For each product
            ...
            if (!orders.containsKey(supplierProduct.getMySupplier())) // If we haven't opened an order to this supplier in the current orders list yet
                orders.put(supplier, new Order(supplier)); // open a new order in the current order list
